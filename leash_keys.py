...
# The code to get key inputs from the user
def get_key():
    # Wait for key press
    key = getch.getch()
...
# This is used to set the drone's target roll, pitch, and yaw.
	...
	while run:
		...
		msg = vehicle.message_factory.set_attitude_target_encode(
			...
			hover_thrust  # thrust (0-1 where 0.5 is no vertical velocity)
               # OK. we might not want to use the hover_thrust - they probably used this for tension
               # se might want to just maintain an altitude
		)
