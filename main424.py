from Drone_Class import (
    Sam4_Drone,
    Simulated_Drone_Simple_Physics,
    DroneMode,
    YawControlMode,
)
...
def run_simulation(
    use_gui,
    drone_inst,
    drone_controller_inst,
    lidar_and_wall_sim_inst,
    walls,
    GUI_inst=None,
):
    """
    ...
    while run_program:
        ...
        if drone_inst.drone_mode == DroneMode.WALL_FOLLOW:
            rpyt = drone_controller_inst.get_target_drone_roll_pitch_yaw_thrust_pid(
        ...
        drone_inst.set_attitude_setpoint(
            rpyt[0], rpyt[1], rpyt[2], rpyt[3], drone_inst.yaw_control_mode
        )
        ...
        if use_gui:
            ...
            GUI_inst.draw_lidar_points(
                lidar_and_wall_sim_inst.get_lidar_readings_angle_deg_dist_m()
            )
            GUI_inst.update_canvas()

        # Display the mode as a string on the terminal
        mode_string = (
            "Follow" if drone_inst.drone_mode == DroneMode.WALL_FOLLOW else "Keys Only"
        )
        yaw_mode = (
            "Yaw Pos"
            if drone_inst.yaw_control_mode == YawControlMode.POSITION
        ...
        print(
            "A: {0:10.3f} D: {1:10.3f}, R: {2:10.3f}, P: {3:10.3f}, Y: {4:10.3f}, T: {5:10.3f}, Mode: {6}, {7}".format(
...
if __name__ == "__main__":
    ...
    walls = [Wall((-4, -4), (-4, 4)), Wall((-4, 4), (0, 4))]
    ...
    lidar_and_wall_sim_inst = Lidar_and_Wall_Simulator(
        walls, lidar_noise_meters_standard_dev
    )
    ...
    move_drone_thread = threading.Thread(
